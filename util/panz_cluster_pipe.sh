...
usage="
--------------------------------------------------
 A script for Clustering PANZ_PIPE outputs
--------------------------------------------------
USAGE: 
    bash $0 <OPTIONS>
...
###################################################
" >&2
echo "bash $srcdir/panz_bed_cls.sh 00_combine-left.bed.gz $unaln_seq_list S1 $pre_distance $coverage $identity $wordlength $threads $keep" >&2
bash $srcdir/panz_bed_cls.sh 00_combine-left.bed.gz $unaln_seq_list S1 $pre_distance $coverage $identity $wordlength $threads $keep
...
bash $srcdir/panz_bed_cls.sh S2_00_combine-right.bed.gz $unaln_seq_list S2 $pre_distance $coverage $identity $wordlength $threads $keep
if [[ $? -ne 0 ]] ; then 
...
bash $srcdir/panz_bed_cls.sh S3_00_all_left_right.bed.gz $unaln_seq_list S3 $final_distance $coverage $identity $wordlength $threads $keep
if [[ $? -ne 0 ]] ; then 
